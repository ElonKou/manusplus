...
namespace Manus
{
	...
	public class CommunicationHub
	{
		...
		{
			public CoreSDK.Timestamp publishTime;
			public List<CoreSDK.ErgonomicsData> data;

			public ErgonomicsStream( CoreSDK.Timestamp p_PublishTime )
			{
		...
		public List<CoreSDK.TemporarySkeletonsInfoForSession> GetTemporarySkeletons()
		{
			...
			CoreSDK.TemporarySkeletonCountForSessions t_TemporarySkeletonCountForSessions = new CoreSDK.TemporarySkeletonCountForSessions();
			...
			for( int i = 0; i < t_TemporarySkeletonCountForSessions.sessionCount; i++ )
			{
		...
		public bool SetupSkeletonTarget( Skeleton p_Skeleton )
		{
			...
			switch( p_Skeleton.skeletonData.settings.targetType )
			{
				case CoreSDK.SkeletonTargetType.Invalid:
					p_Skeleton.skeletonData.settings.targetType = CoreSDK.SkeletonTargetType.UserIndexData;
					p_Skeleton.skeletonData.settings.skeletonTargetUserIndexData.index = 0;
					break;
				case CoreSDK.SkeletonTargetType.UserData:
					if( p_Skeleton.skeletonData.settings.skeletonTargetUserData.id == 0 )
					{
						...
						p_Skeleton.skeletonData.settings.skeletonTargetUserData.id = t_UserIDs[0];
						return true;
					}
					break;
		...
		protected void SendCustomTracker()
		{
			...
			foreach( var t_Tracker in m_CustomTrackers )
			{
				...
				if( t_Result == CoreSDK.SDKReturnCode.Success )
				{
					m_CustomTrackers[t_Tracker.Key].type = t_TrackerData.type;
