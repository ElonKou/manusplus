...
namespace Manus.Editor
{
	...
	public class SkeletonDataEditor : UnityEditor.Editor
	{
		...
		Vector2 m_MeshScrollPosition = Vector2.zero;
		...
		public override void OnInspectorGUI()
		{
			...
			EditorGUILayout.BeginVertical( "box" );
			{
				...
				{
					case CoreSDK.SkeletonType.Invalid:
						break;
					case CoreSDK.SkeletonType.Hand:
						break;
					case CoreSDK.SkeletonType.Body:
					case CoreSDK.SkeletonType.Both:
						break;
					default:
						break;
				}
			...
			t_Skeleton.skeletonData.settings.targetType );
				...
				switch( t_SkeletonTargetType )
				{
					...
					case CoreSDK.SkeletonTargetType.UserData:
						t_Skeleton.skeletonData.settings.skeletonTargetUserData.id = (uint)EditorGUILayout.DelayedIntField( new GUIContent( "User ID", "The ID of the user that this skeleton should use to animate." ),
							(int)t_Skeleton.skeletonData.settings.skeletonTargetUserData.id );
						break;
					case CoreSDK.SkeletonTargetType.UserIndexData:
						t_Skeleton.skeletonData.settings.skeletonTargetUserIndexData.index = (uint)EditorGUILayout.DelayedIntField(
							new GUIContent( "User Index", "The index of the User that this skeleton should use to animate." ),
							(int)t_Skeleton.skeletonData.settings.skeletonTargetUserIndexData.index );
						break;
					...
					case CoreSDK.SkeletonTargetType.GloveData:
						t_Skeleton.skeletonData.settings.skeletonTargetGloveData.id = (uint)EditorGUILayout.DelayedIntField( new GUIContent( "Glove ID", "The ID of the glove that this skeleton should use to animate." ),
							(int)t_Skeleton.skeletonData.settings.skeletonTargetGloveData.id );
						break;
				}
			}
			EditorGUILayout.EndVertical();
		...
		void DisplayChainSettings( Skeleton p_Skeleton, Chain p_Chain )
		{
			...
			switch( t_ChainType )
			{
				...
				case CoreSDK.ChainType.Hand:
					...
					p_Chain.settings.hand.handMotion =
						(CoreSDK.HandMotion)EditorGUILayout.EnumPopup( new GUIContent( "Hand Motion Type", "Tracking source of hand, will follow skeleton if set to none." ),
							p_Chain.settings.hand.handMotion );
					break;
